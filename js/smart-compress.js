// SmartCompress: Intelligent, URL-safe, lossless text compression for share links
// Strategies:
//  - 'lu': LZString.compressToEncodedURIComponent (URL-safe)
//  - 'lb': LZString.compressToBase64 + base64url (often slightly different density)
//  - 'w1': Word-Dictionary preconditioning + LZ URI (content-aware, helps repetitive long words)
// Chooses the smallest result. Lossless by design.
(function(){
  if (!window.LZString) { console.warn('SmartCompress: LZString not found'); return; }
  ...
  // Build a frequency dictionary of words; keep only beneficial entries (len >=4 and frequency>=3)
  function buildWordDict(text, maxWords=512) {
    // Unicode words; fallback to ASCII if unsupported
  ...
  // Encode text by replacing dict words with compact tokens ~<base36>.
  // Only replace when token is strictly shorter than the word to ensure non-inflation.
  function encodeWithDict(text, dict){
    if (!dict || dict.length === 0) return { encoded: text, replaced: 0 };
    const index = new Map(dict.map((w,i)=>[w,i]));
    // Sort by length desc to avoid partial overlaps (longer words first)
    ...
    const encoded = text.replace(pattern, (match) => {
      ...
      if (i === undefined) return match;
      const tok = '~' + i.toString(36) + '.'; // token marker with dot terminator
      if (tok.length < match.length) { replaced++; return tok; }
      return match;
    });
  ...
  function decodeWithDict(encoded, dict){
    if (!dict || dict.length === 0) return encoded;
  ...
  // Lightweight type detection to guide telemetry and thresholds
  function detectType(text){
    ...
    // HTML/XML
    if (/<[a-zA-Z!\/?][\s\S]*>/m.test(t)) return 'html';
    // Code-like (JS/CSS)
    if (/(function|=>|const |let |var |class |;\s*$|\{[\s\S]*\}|:[^;]+;)/m.test(t)) return 'code';
    // Language-like
    if (/\b(?:el|la|de|que|and|the|para|con|por|you|este|esto)\b/i.test(t)) return 'lang';
    return 'other';
  }

  // Telemetry store (localStorage) for adaptive thresholds per type
  const TELEMETRY_KEY = 'SC.telemetry.v1';
  function loadTelemetry(){
    try { const raw = localStorage.getItem(TELEMETRY_KEY); return raw ? JSON.parse(raw) : { data: {} }; } catch { return { data: {} }; }
  }
  function saveTelemetry(state){ try { localStorage.setItem(TELEMETRY_KEY, JSON.stringify(state)); } catch {} }
  function recordTelemetry(type, bestScheme, gain){
    ...
    rec.total += 1;
    if (rec.best[bestScheme] != null) rec.best[bestScheme] += 1;
  ...
  function encodeWithPrefix(text, prefixes){
    if (!prefixes || prefixes.length===0) return { encoded: text, replaced: 0 };
    ...
    const out = text.replace(pat, (full, pre, stem) => {
      ...
      if (tok.length < pre.length){ replaced++; return tok + stem; }
      return full;
    });
    return { encoded: out, replaced };
  }
  function decodePrefix(encoded, prefixes){
    if (!prefixes || prefixes.length===0) return encoded;
    return encoded.replace(/\^([0-9a-z]+)`/g, (m,g1)=>{
  ...
  // Build a suffix dictionary (affix) for frequent endings, length 3-6
  ...
  function encodeWithSuffix(text, suffixes){
    if (!suffixes || suffixes.length===0) return { encoded: text, replaced: 0 };
    ...
    const out = text.replace(pat, (full, stem, suf) => {
      const i = idx.get(suf);
      const tok = '`' + i.toString(36) + '~';
      if (tok.length < suf.length){ replaced++; return stem + tok; }
      return full;
    });
    return { encoded: out, replaced };
  }
  function decodeSuffix(encoded, suffixes){
    if (!suffixes || suffixes.length===0) return encoded;
    return encoded.replace(/`([0-9a-z]+)~/g, (m,g1)=>{
      const i = parseInt(g1,36);
  ...
  function evaluate(text){
    ...
    // Strategy B: LZ Base64 then URL-safe
    try {
    ...
    // Strategy C: WordDict + optional Prefix/Suffix + optional reversible stemming then LZ URI (only for sizable text)
    try {
  ...
  // Optional reversible stemming (tiny rule set)
        ...
        const applyStemming = (src)=>{
          ...
          if (stemAffixes.length===0) return { out: src, replaced: 0, dict: [] };
          ...
          const out = src.replace(pat, (full, base, aff)=>{
            ...
            const tok = '%' + i.toString(36) + '!';
            if (tok.length < aff.length){ replaced++; return base + tok; }
            return full;
          });
          return { out, replaced, dict: stemAffixes };
        };
        // Avoid stemming for 'code' or 'html' unless it reduces size significantly vs pre-stem
        let allowStemming = true;
        const contentType = detectType(text);
        if (contentType === 'code' || contentType === 'html') allowStemming = false;
        let stemRes = { out: afterSuffix, replaced: 0, dict: [] };
        if (allowStemming) {
          stemRes = applyStemming(afterSuffix);
          // Require at least modest improvement vs LU and vs afterSuffix (2% locally)
        ...
        stemmed = stemRes.out; stemRepl = stemRes.replaced; stemDict = stemRes.dict;
        ...
        if (replaced + preRepl + sufRepl + stemRepl >= CONFIG.w1.minReplaced) {
          ...
          // ensure real gain vs LU if LU exists
  ...
  function decompressByScheme(scheme, payload){
    ...
    try {
      ...
      if (scheme === 'w1') {
        ...
        if (!parts[0].startsWith('W1')) return composite;
        // Back-compat: 3 parts (word|encoded), 4 parts (word|suffix|encoded)
        // New: 6 parts (word|prefix|suffix|stem|encoded)
        const wordB64u = parts[1];
        let prefixes = [], suffixes = [], stems = [];
        let encoded = '';
        if (parts.length >= 6){
          try { const b64p = b64urlDecode(parts[2]); const preStr = decodeURIComponent(escape(atob(b64p))); prefixes = preStr ? preStr.split('\u0001') : []; } catch {}
          try { const b64s = b64urlDecode(parts[3]); const sufStr = decodeURIComponent(escape(atob(b64s))); suffixes = sufStr ? sufStr.split('\u0001') : []; } catch {}
          try { const b64t = b64urlDecode(parts[4]); const stmStr = decodeURIComponent(escape(atob(b64t))); stems = stmStr ? stmStr.split('\u0001') : []; } catch {}
          encoded = parts.slice(5).join('|');
        } else if (parts.length >= 4){
          try { const b64s = b64urlDecode(parts[2]); const sufStr = decodeURIComponent(escape(atob(b64s))); suffixes = sufStr ? sufStr.split('\u0001') : []; } catch {}
          encoded = parts.slice(3).join('|');
        } else {
          encoded = parts[2];
        }
        let dict = [];
        try { const b64w = b64urlDecode(wordB64u); const dictStr = decodeURIComponent(escape(atob(b64w))); dict = dictStr ? dictStr.split('\u0001') : []; } catch {}
        // Decode in reverse order of encode: stemming -> suffix -> prefix -> words
  ...
  // Optional memoization using LRU if available
  const cache = (function(){
    const LRU = window.Algos && window.Algos.LRUCache ? window.Algos.LRUCache : null;
    if (LRU) return new LRU(20);
    // simple fallback
    const m = new Map();
    return {
      get(k){ return m.get(k); },
      set(k,v){ m.set(k,v); if (m.size>20){ const it=m.keys().next().value; m.delete(it);} }
    };
  })();
