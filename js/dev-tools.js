...
ORDER BY month DESC;`;
    ...
    static calculateTextComplexity(text) {
        ...
        metrics.readabilityScore = Math.max(0, 206.835 - (1.015 * avgSentenceLength) - (84.6 * (avgWordLength / 5))).toFixed(1);
        if (metrics.readabilityScore >= 90)
            metrics.complexity = 'Very Easy';
        else if (metrics.readabilityScore >= 80)
            metrics.complexity = 'Easy';
        else if (metrics.readabilityScore >= 70)
            metrics.complexity = 'Fairly Easy';
        else if (metrics.readabilityScore >= 60)
            metrics.complexity = 'Standard';
        else if (metrics.readabilityScore >= 50)
            metrics.complexity = 'Fairly Difficult';
        else if (metrics.readabilityScore >= 30)
            metrics.complexity = 'Difficult';
        else
            metrics.complexity = 'Very Difficult';
        return metrics;
    }
}
class CodeFeatures {
    static getAutocomplete(text, cursorPosition) {
        ...
        return matches.filter(suggestion => suggestion.toLowerCase().startsWith(currentWord.toLowerCase()));
    }
