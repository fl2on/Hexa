(function () {
    ...
    const Profile = {
        ...
        save(p) { Safe.set(storeKey, p, 1000 * 60 * 60 * 24 * 30); }
    };
    const AutoOpt = {
        profile: null,
        _panelOpenSince: {},
        ...
        trackThemeChange(dark) {
            try {
                ...
                if (dark && (hour >= 19 || hour <= 6))
                    this.profile.theme.darkAtNight++;
                if (!dark && (hour >= 9 && hour <= 17))
                    this.profile.theme.lightAtDay++;
                Profile.save(this.profile);
            }
            catch { }
        },
                ...
                if (app && sugg.autosaveMs && typeof app.autoSaveInterval !== 'undefined') {
                    ...
                    app.autoSaveInterval = setInterval(() => {
                        try {
                            ...
                            if (app.text !== last) {
                                if (window.TextStore) {
                                    window.TextStore.set(app.text);
                                }
                                else {
                                    try {
                                        localStorage.setItem('text', app.text);
                                    }
                                    catch { }
                                }
